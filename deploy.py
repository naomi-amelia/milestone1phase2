...
st.header('Telco Customer Churn Predictor')
...
PaymentMethod = st.selectbox('Subcription payment method', ['Electronic check', 'Mailed check', 'Bank transfer (automatic)', 'Credit card (automatic)'])
...
StreamingTV = st.selectbox('TV streaming service', ['Yes', 'No', 'No internet service'])
StreamingMovies = st.selectbox('Movies streaming service', ['Yes', 'No', 'No internet service'])
InternetService = st.selectbox('Internet service', ['DSL', 'Fiber optic', 'No internet service'])
...
if st.button('submit'):
    ...
    num_df = pd.DataFrame([[tenure]], columns=num_cols)

    cat_df = pd.DataFrame([[SeniorCitizen,Partner, Dependents, MultipleLines, InternetService,
               OnlineSecurity, OnlineBackup, DeviceProtection, TechSupport,
               StreamingTV, StreamingMovies, Contract, PaperlessBilling,
